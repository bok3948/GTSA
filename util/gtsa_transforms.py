...
class RandomRotationReturnsDegree:
...
def rotate_bbox(bbox, angle, img_size):
    # 원래 좌표 상자의 좌표값을 가져옵니다
    x1, y1, x2, y2 = bbox
    
    # 회전할 때 새로운 좌표값을 저장할 변수를 초기화합니다
    new_x1, new_y1, new_x2, new_y2 = 0, 0, 0, 0
    
    # 회전하는 경우에 따라서 좌표값을 계산합니다
    if angle == 0:
        new_x1, new_y1, new_x2, new_y2 = x1, y1, x2, y2
    elif angle == 90:
        new_x1, new_y1, new_x2, new_y2 = y1, img_size - x2, y2, img_size - x1
    elif angle == 180:
        new_x1, new_y1, new_x2, new_y2 = img_size - x2, img_size - y2, img_size - x1, img_size - y1
    elif angle == 270:
        new_x1, new_y1, new_x2, new_y2 = img_size - y2, x1, img_size - y1, x2
    else:
...
class GTSATransforms:
    ...
    def __init__(self,
        ...
        color_transform.append(torchvision.transforms.RandomApply([blur], p=0.2))
        ...
        self.RandomRotationReturnsDegree = RandomRotationReturnsDegree()
    def __call__(self, sample: torch.Tensor) -> Tuple[List[Tensor], Dict[str, Tensor]]:
        ...
        for i, rrc_transform in enumerate(self.rrc_transforms):
            ...
            img, angle = self.RandomRotationReturnsDegree(img)
        ...
        gc_bboxes, otc_bboxes = self.calculate_bboxes(crop_bboxes, angles)
        angles = [torch.tensor(angle, dtype=torch.float16) for angle in angles]

        return multi_crops, {"gc": gc_bboxes, "all": otc_bboxes, "angles": angles}

    def calculate_bboxes(self, crop_bboxes: Tensor, degrees=None):
        ...
        for i in range(otc_bboxes.shape[0]):
            for j in range(otc_bboxes.shape[1]):
                
                if i < self.nmb_crops[0]:
                    size = self.size_crops[0]
                else:
                    size = self.size_crops[1]
                otc_bboxes[i][j] = rotate_bbox(otc_bboxes[i][j], degrees[i], size)
                
        for i in range(gc_bboxes.shape[0]):
            for j in range(gc_bboxes.shape[1]):
                
                if i < self.nmb_crops[0]:
                    size = self.size_crops[0]
                else:
                    size = self.size_crops[1]
                gc_bboxes[i][j] = rotate_bbox(gc_bboxes[i][j], degrees[i], size)
